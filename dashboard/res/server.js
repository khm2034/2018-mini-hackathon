...
var PANO_FRAME_FILE_PATH = '/opt/usr/home/owner/apps_rw/org.tizen.smart-surveillance-camera/shared/data/pano.jpg'
...
if (ENABLE_WEBSOCKET) {
	  ...
	  var websocket2 = require('websocket');

	  var options2 = {
	    port: 8989
	  }

	  var server = new websocket2.Server(options2, Listener2);

	  function Listener2(ws) {
	    console.log('Client connected: handshake done!');
	    ws.ack = true;
	    ws.on('message', function (msg) {
	      console.log('Message received: %s', msg.toString());
	    ...
	    var timeout = setInterval(function() {
	      ...
	      var data;
	      try {
	       data = fs.readFileSync(PANO_FRAME_FILE_PATH);
	      } catch (err) {
	    	console.log(err);
	        data = fs.readFileSync(SERVER_ROOT_FOLDER_PATH + 'default.gif');
	      }
